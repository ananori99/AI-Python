...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # P(noisyDistance | TrueDistance) * P(TrueDist) Slide 52, of uncertanty 1 Bayes

        # Try printing noisyDistance, emissionModel, and PacmanPosition (in the observe function) to get started.
        # print(noisyDistance) # int
        # print(emissionModel) # set with mappings TDist: P(noisey)
        # print(pacmanPosition) # (x,y)
        # print(self.beliefs)  # map of positon to all possible belived states and their probability

        # Replace this code with a correct observation update
        # Be sure to handle the "jail" edge case where the ghost is eaten
        allPossible = util.Counter()
        # if a ghost is eaten, then the noisy distance is None and the ghost is 100% in jail
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:  # as in if ghost not caught
                # util.Counter objects (like dictionaries) in a field called self.beliefs, which you should update.
                allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]  # CHANGE HERE
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Since Pacman is not utilizing any observations about the ghost, this means that Pacman will start
        # with a uniform distribution over all spaces, and then update his beliefs according to
        # how he knows the Ghost is able to move.
        allPossible = util.Counter()
        # if a ghost is eaten, then the noisy distance is None and the ghost is 100% in jail

        # newPosDist is a util.Counter object, where for each position p in self.legalPositions
        # inference by enumeration: need the CPTs to construct all possible ways
        # query could be true (and sum them)
        # for all t, get all probs at t+1
        # final belief in t+1 is sum of how much all t believe t+1 is new Pos, proportinate to that t's own prob
        # inference by enumeration in BN slides
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # You may also find it useful to loop over key, value pairs
            #         in newPosDist, like
            for newPos, prob in newPosDist.items():  # how likely for the prior dist's ways to get there?
                # inference by enumeration: need the CPTs to construct all possible ways
                # query could be true (and sum them)
                # Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )*Pr(ghost at oldPos)
                # for all the possible new postions
                allPossible[newPos] = allPossible[newPos] + prob * self.beliefs[oldPos]  # CHANGE HERE
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Particles should be evenly (not randomly) distributed across positions in order to
        #  ensure a uniform prior.
        # as in, all the positions should appear an equal number of times in the particle list?
        rep = nP / len(pos)
        # print(rep)
        self.particles = rep * pos
        append = nP - len(self.particles)
        if append:
            self.particles.extend(pos[0:append])
    ...
    def observe(self, observation, gameState):
        ...
        # modifying exact
        allPossible = util.Counter()
        # because in this class there are only particles, not a self.beliefs
        beliefs = self.getBeliefDistribution()

        # when a ghost is captured the nosey distance is none,
        # and you are certain it is in jail
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:  # as in if ghost not caught
                    # util.Counter objects (like dictionaries) in a field called self.beliefs, which you should update.
                    allPossible[p] = emissionModel[trueDistance] * beliefs[p]  # CHANGE HERE
            ...
            # call total count for weight, and recreate if zero
            # otherwise, create the new particles by "sampleing" the belif distribution
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                # create iterable
                # dist = allPossible.normalize()
                # print(allPossible.totalCount())
                # print(util.sample(allPossible))
                # this
                self.particles = [util.sample(allPossible) for a in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        # in terms of particles, for each current particle, "sample" to see where it is likely to go
        self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, p))) for p in
                          self.particles]
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # basically just see the frequency each occurs at over the total
        # counter is basically a dictionary
        for p in self.particles:
            allPossible[p] += 1
        allPossible.normalize()  # turn from count to probability
        return allPossible
        util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        import itertools
        # Look at the 'product' function in itertools to get an
        # implementation of the catesian product.
        # all the positions ghosts can occupy for N ghosts, ghosts can be on same square
        # https://thomas-cokelaer.info/blog/2012/11/how-do-use-itertools-in-python-to-build-permutation-or-combination/
        cProd = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        # Note: If you use itertools, keep in mind that permutations are not returned in a random order;
        # you must shuffle the list https://pynative.com/python-random-shuffle/
        random.shuffle(cProd)
        m = len(cProd)
        print(m)

        # these tuples are now each a particle, so same as q4 now
        self.particles = [cProd[i % m] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # because in this class there are only particles, not a self.beliefs
        #beliefs = self.getBeliefDistribution()
        # what do the given look like?
        # print(noisyDistances) # this is an array of a distance per ghost
        # print(emissionModels) # array of dictionaries per ghost

        # if a ghost is eaten, all particles should update to have that  ghost in it's cell
        # as in for a particle, all elements at that position should become the jail
        # as the other particles are unasigned, it may be better to create a list for use by filter
        ghostState = self.numGhosts * [0]

        for i in range(self.numGhosts):
            if noisyDistances[i] is None:
                ghostState[i] = 1
                # self.particles = [self.getParticleWithGhostInJail(p, i) for p in self.particles]
                # self.particles = [self.getParticleWithGhostInJail(self.particles[j], i) for j in range(self.particles)
                # filter(ghostState[i])]

        for p in self.particles:
            # then set  ghosts particles for jailed
            weight = 1
            # you are 100% sure ghost is in jail
            for g in range(self.numGhosts):
                # if in jail, set all the particles to have that (x,y) be jail
                if ghostState[g]:
                    for p in self.particles:
                        p = self.getParticleWithGhostInJail(p, g)



            # otherwise, not in jail
            # hmm slide p47, start all particles with weight 1. Take the prodocut of the P(newState) from the transition model
            # find each part's weight, starting it at 1
            for g in range(self.numGhosts):
                if not ghostState[g]:
                    # weight = P(emission|state), where state is now a tuple of ghost pos. Maybe wrong here,
                    # independent?
                    # weight = [(emissionModels[j])[util.manhattanDistance(p[i], pacmanPosition)] for j in range(self.numGhosts)]
                    # print(weight, sum(weight))
                    # then sum P(xt)*B(xt)
                    # weight = functools.reduce(lambda x, y: x * y, weight)
                    trueDistance = util.manhattanDistance(p[g], pacmanPosition)
                    weight = weight * (emissionModels[g])[trueDistance]
